...
export default function CartCard(props: ICartProductCard) {
  const { images, title, price, discount, isFavorite, id } = props;
  ...
  useEffect(() => {
    let tempArr = cartProducts.filter((item) => item.title === title);
    if (tempArr.length > 0) {
    ...
    >
      <div className="flex items-center gap-5 pl-10">
        <div className="relative">
          <Image
            alt={title}
            src={images && images.length > 0 ? images[0] : "/images/placeholder.webp"} // Corrected image URL
            width={100}
            height={100}
          ...
          <div
            ...
            onClick={async () => {
              ...
              try {
                await removeProductFromCart(props.id, token);
                setAmount(0);
              } catch (error) {
