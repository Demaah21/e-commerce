  ...
  function handleClick() {
    setCheckoutProducts(
      cartProducts.map((item) => {
        const { images, title, price, discount, amount } = item;
        ...
        return { image: images[0], name: title, price: calculatedPrice };
      })
    );
  }
          ...
          buttonProps={{
            onClick: async () => {
              ...
              try {
                ...
                setCartProducts([]); // Clear local cart state
              } catch (error) {
                console.error("Failed to clear cart:", error);
              }
            },
          }}
          className="self-center bg-red-500 hover:bg-red-600"
        >
          Clear Cart
        </PrimaryButton>
      </div>
